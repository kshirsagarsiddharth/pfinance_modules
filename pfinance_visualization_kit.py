...
df = pd.read_csv("../IT.NS.CSV")
IT_COMPANY_LIST = ['MPHASIS.NS', 'COFORGE.NS', 'MINDTREE.NS', 'INFY.NS', 'TECHM.NS',
       'LTI.NS', 'HCLTECH.NS', 'TCS.NS', 'WIPRO.NS', 'LTTS.NS']


def select_time_period(df,code):
    """
...
def pick_company(company_name, time_period_start,time_period_end, time_offset):
    """
    :param company_name: company to pick 
    :param time_period: time period used for visualization
    """
    df2 = df[df['ticker'] == company_name]
    df2['Date'] = pd.to_datetime(df2['Date'])
   
    df2 = df2.set_index('Date')
    df2 = select_time_period(df2, time_offset)
...
def display_vizs():
    controls = widgets.interact(pick_company,
                            company_name = widgets.Dropdown(options = IT_COMPANY_LIST,index = 2),
