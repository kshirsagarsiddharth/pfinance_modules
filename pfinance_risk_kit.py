...
def bond_cash_flow(maturity = 4, principal = 100, coupon_rate = 0.03, coupons_per_year = 12): 
    """
    Returns a series of cash flows generated by a bond, 
    indexed by coupon number
    
    :param maturity: Number of years after which the coupon will mature 
    :param principal: The principal amount we invested to get this return 
    :param coupon_rate: The intrest rate this coupon bearing bond will return per year
    :param coupons_per_year: number of coupons this bond will generate per year
    :returns float:
    """
    n_coupons = round(maturity * coupons_per_year)  
    coupon_amount = (principal * coupon_rate) / coupons_per_year 
    coupon_times = np.arange(1, n_coupons + 1) 
    cash_flow = pd.Series(data = coupon_amount, index = coupon_times) 
    cash_flow .iloc[-1]+= principal
    return cash_flow 


def bond_adjusted_price(maturity = 4, principal = 100, coupon_rate = 0.03, coupons_per_year = 12, discount_rate = 0.03):
    """
    Price a bond based on bond parameters maturity, principal coupon rate and coupons_per_year 
    and prevailing discounted rate
    
    :param maturity: Number of years after which the coupon will mature 
    :param principal: The principal amount we invested to get this return 
    :param coupon_rate: The intrest rate this coupon bearing bond will return per year
    :param coupons_per_year: number of coupons this bond will generate per year
    :param discount_rate: This is annual discount rate 
    :returns present value of bond discounted:
    """
...
def macaulay_duration(cash_flows, discount_rate): 
    """
    More the value of macaulay_duration the more is the initial intrest risk
    Computes the macaulay duration of a sequence of cashflows
    sum(<discounted_cashflow> * <time_period> / sum(<discounted_cash_flows>))
    
    if two portfolios have same macaulay duration there intrest rate risk is same or in other words 
    if we can match macaulay duration of bonds to the macaulay duration of liabilities we get 2 portfolios which have same sensitivity to intrest rate 
    """
    discounted_cashflow = cash_flows * discount(cash_flows.index,discount_rate) 
    return ((discounted_cashflow* cash_flows.index) / np.sum(discounted_cashflow)).sum() 

def match_macaulay_duration(cash_flow_longterm_bond, cash_flow_short_term_bond, cash_flow_liabilities, discount_rate): 
    """
    <weight_short_term> * <macaulay_duration_short_term> + <weight_long_term> * <macaulay_duration_long_term> = <macaulay_duration_liabilities>
    
    returns weight_short_term i.e weight to be given to short term bond to match macaulay ratios
    
    <weight_short_term> = (macaulay_duration_long_term - macaulay_duration_liabilities) / (macaulay_duration_short_term - macaulay_duration_liabilities)
    """
    macaulay_duration_short_term = macaulay_duration(cash_flow_short_term_bond, discount_rate)
    macaulay_duration_long_term = macaulay_duration(cash_flow_longterm_bond, discount_rate) 
    macaulay_duration_liabilities = macaulay_duration(cash_flow_liabilities, discount_rate) 
    
    return (macaulay_duration_long_term - macaulay_duration_liabilities) / (macaulay_duration_long_term - macaulay_duration_short_term)
